...
// NOVO: Carregar menu do localStorage (sincronizado com o painel admin)
function loadMenuItems() {
    const savedMenu = localStorage.getItem('restaurante24h_menu');
    
    if (savedMenu) {
        // Se existir dados salvos pelo admin, usa eles
        return JSON.parse(savedMenu);
    } else {
        // Primeira vez: salva os dados padrão
        localStorage.setItem('restaurante24h_menu', JSON.stringify(defaultMenuItems));
        return defaultMenuItems;
    }
}

// Carregar itens do menu (agora vem do localStorage)
...
// NOVO: Verificar atualizações do menu periodicamente
function checkMenuUpdates() {
    const savedMenu = localStorage.getItem('restaurante24h_menu');
    if (savedMenu) {
        const newMenuItems = JSON.parse(savedMenu);
        // Verifica se houve mudanças
        if (JSON.stringify(newMenuItems) !== JSON.stringify(menuItems)) {
            menuItems = newMenuItems;
            renderMenuItems(); // Atualiza a exibição
            console.log('📋 Menu atualizado do painel admin!');
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('🍽️ Restaurante 24h carregado!');
    console.log('📋 Menu sincronizado com o painel admin');
});
